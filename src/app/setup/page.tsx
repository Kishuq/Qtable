  ...
  return (
    <div className="mx-auto flex w-full max-w-md flex-1 flex-col justify-center px-5 py-14">
      <div className="glass rounded-3xl p-8">
        ...
        <h1 className="mt-2 text-2xl font-black">Set up your cafe</h1>
        <p className="mt-1 text-sm text-stone-400">One-time, 60 seconds. Menu, tables & QRs are created for you.</p>
        <form onSubmit={submit} className="mt-6 space-y-3">
          <input value={form.cafeName} onChange={set("cafeName")} required placeholder="Cafe name (e.g. Brew Haven)" className="w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm outline-none focus:border-orange-500" />
          ...
          <input value={form.email} onChange={set("email")} type="email" required placeholder="owner@cafe.com (your login)" className="w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm outline-none focus:border-orange-500" />
          ...
          <button disabled={loading} className="w-full rounded-2xl bg-gradient-to-r from-orange-600 to-amber-600 py-3 font-black disabled:opacity-60">
            {loading ? "Opening your cafe… 🎉" : "Open my cafe →"}
          </button>
        </form>
